...
@ManagedBean
@SessionScoped
public class PickSecurityInfoBean implements Serializable {
    ...
    public void search() {
        if(nin == null || nin.trim().isEmpty()){
            ...
            message.setSummary("Необходимо ввести НИН!");
            FacesContext.getCurrentInstance().addMessage(null, message);
            return;
        }

        List<RefSecuritiesItem> securities = sessionBean.getReference().getSecuritiesByNin(sessionBean.getIntegration().getNewDateFromBackEndServer(), nin.toUpperCase());
        security = securities.size() > 0 ? securities.get(0) : null;
