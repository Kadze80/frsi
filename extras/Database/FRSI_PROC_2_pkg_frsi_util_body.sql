CREATE or replace  PACKAGE BODY "PKG_FRSI_UTIL" AS
  ...
  AS
    ...
    BEGIN
      ...
      CASE period_code
        WHEN 'w' THEN
        IF (to_char(trunc(report_date), 'D') - 1) = 1 THEN Result := 1; ELSE Result := 0; END IF;
        WHEN 'm' THEN
  ...
  AS
    ...
    BEGIN
      ...
      v_year := extract(YEAR FROM date_);
      v_month := extract(MONTH FROM date_);
      v_day := extract(YEAR FROM date_);

      CASE upper(pt)
        WHEN 'Y'
        THEN result := trunc(date_, 'YEAR');
        WHEN 'H'
        THEN
          h := trunc(v_month / 6);
          IF (mod(v_month, 2) != 0)
          THEN h := h + 1; END IF;
          hm := h * 6 - 5;
          result := trunc(add_months(date_, -1 * (v_month - hm)), 'MONTH');
        WHEN 'Q'
        THEN
          q := trunc(v_month / 3);
          IF (mod(v_month, 3) != 0)
          THEN
            q := q + 1; END IF;
          qm := q * 3 - 2;
          result := trunc(add_months(date_, -1 * (v_month - qm)), 'MONTH');
        WHEN 'M'
        THEN
          result := trunc(date_, 'MONTH');
        WHEN 'W'
        THEN
          result := trunc(date_, 'W');
      ELSE
        result := date_;
      END CASE;
  ...
  AS
    ...
    BEGIN
      ...
      CASE upper(pt)
        WHEN 'Y'
        THEN result := add_months(date_, 12 * period_count);
        WHEN 'H'
        THEN
          result := add_months(date_, 6 * period_count);
        WHEN 'Q'
        THEN
          result := add_months(date_, 3 * period_count);
        WHEN 'M'
        THEN
          result := add_months(date_, period_count);
        WHEN 'W'
        THEN
          result := date_ + period_count*7;
        WHEN 'D'
        THEN
          result := date_ + period_count;
      ELSE
        result := date_;
      END CASE;
