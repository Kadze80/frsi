create or replace package body pkg_frsi_ref_load is
  ...
  begin
    Err_Code := 0;
    Err_Msg  := 'Справочник загружен успешно!';
...
--               t.delfl = Rec.Delfl,
  ...
  exception
    when others then
      rollback;
      Err_Code := SQLCODE;
      Err_Msg  := ProcName || ' Ошибка обновления справочника "Банки"!';
  end;
...
--               t.delfl = Rec.Delfl,
  ...
  exception
    when others then
      rollback;
      Err_Code := SQLCODE;
      Err_Msg  := ProcName || ' Ошибка обновления справочника "Рейтинговых агенств"!';
  end;
...
--               t.delfl = Rec.Delfl,
  ...
  exception
    when others then
      rollback;
      Err_Code := SQLCODE;
      Err_Msg  := ProcName || ' Ошибка обновления справочника "Валюты"!';
  end;
...
--               t.delfl = Rec.Delfl,
  ...
  exception
    when others then
      rollback;
      Err_Code := SQLCODE;
      Err_Msg  := ProcName || ' Ошибка обновления справочника "Рейтинг валют"!';
  end;
...
--               t.delfl = Rec.Delfl,
  ...
  exception
    when others then
      rollback;
      Err_Code := SQLCODE;
      Err_Msg  := ProcName || ' Ошибка обновления справочника "Города/Регионы"!';
  end;
...
--               t.delfl = Rec.Delfl,
  ...
  exception
    when others then
      rollback;
      Err_Code := SQLCODE;
      Err_Msg  := ProcName || ' Ошибка обновления справочника "Страны"!';
  end;
...
--               t.delfl = Rec.Delfl,
  ...
  exception
    when others then
      rollback;
      Err_Code := SQLCODE;
      Err_Msg  := ProcName || ' Ошибка обновления справочника "Подразделения НБ РК"!';
  end;
  ...
  begin
    ...
    for Rec in (select ss.rec_id as id,
                 ...
                 where ss.s_issuer = si.id
                   ...
                   and sv.status = 1
                   and st.status = 1
                   and ss.rec_id not in (12695,13862) -- убрал две бумаги с уже имеющеемся НИНами.
                minus
                select s.id,
                       s.rec_id,
                       s.s_issuer,
  ...
  exception
    when others then
      rollback;
      Err_Code := SQLCODE;
      Err_Msg  := ProcName || ' Ошибка обновления справочника "Ценные бумаги"!';
  end;
